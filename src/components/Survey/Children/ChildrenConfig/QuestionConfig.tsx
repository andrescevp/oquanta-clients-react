    ...
    if (!selectedQuestion) {
        return (
            <div className='h-full flex flex-col items-center justify-center p-8 text-gray-500 dark:text-gray-400 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800'>
                ...
                <p className='text-lg font-medium text-center'>{t('Select a question to configure')}</p>
    ...
    return (
        <div className='h-full flex flex-col'>
            ...
            <div className='flex-1 overflow-y-auto p-4 space-y-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800'>
                ...
                <OptionsFormFactory element={selectedQuestion} onChange={handleOptionsChange} />
                    ...
                    <div className='flex items-start'>
                        ...
                        <div>
                            ...
                            <p className='mt-1 text-sm text-gray-500 dark:text-gray-400'>
                                {selectedQuestion.type === 'open_end' &&
                                    t('Text questions allow free-form text responses from survey participants.')}
                                {selectedQuestion.type === 'choice' &&
                                    t('Choice questions provide options for participants to select from.')}
                                {selectedQuestion.type === 'block' &&
                                    t('Blocks are containers that group related questions together.')}
                                {selectedQuestion.type === 'loop' &&
                                    t('Loops repeat a set of questions based on conditions or fixed iterations.')}
                            </p>
                        </div>
                    </div>
                </div>
