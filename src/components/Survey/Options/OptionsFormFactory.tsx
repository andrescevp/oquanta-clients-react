...
import { Block, Loop, QuestionChoice, SurveyRequestChildrenInner } from '../../../api-generated';
...
import OpenEndOptionsForm from './OpenEndOptionsForm';
 ...
 */
const OptionsFormFactory: React.FC<OptionsFormFactoryProps> = ({ element, onChange, className }) => {
    ...
    const renderOptionsForm = () => {
        switch (element.type) {
            case 'open_end':
                return <OpenEndOptionsForm element={element as never as QuestionChoice} onChange={onChange} />;

            case 'choice':
                return <ChoiceOptionsForm element={element as never as QuestionChoice} onChange={onChange} />;
            ...
            default:
                return (
                    <div className='flex flex-col items-center justify-center p-8 text-gray-500 bg-white/60 dark:bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-200 dark:border-gray-700'>
                        ...
                        <p className='text-lg font-medium text-center'>{t('Unsupported Question Type')}</p>
                        <p className='text-sm text-center mt-2 max-w-md'>
                            {t('Options editor not available for type:')}{' '}
    ...
    return <div className={cn('w-full', className)}>{renderOptionsForm()}</div>;
};
