...
const QuestionTree: React.FC<QuestionTreeProps> = ({ items, onItemsChange, onItemSelect, selectedItemId }) => {
    ...
    const renderTreeItems = (treeItems: QuestionItem[], parentId: string | null = null) => {
                ...
                {itemsAtLevel.map(item => {
                    ...
                    return (
                        <React.Fragment key={item.uniqueId}>
                            <div className='my-2'>
                                <DropIndicator
                                    isActive={isItemTarget && dropPosition === 'before' && activeId !== item.uniqueId}
                                />
                            </div>
                            ...
                            {canHaveChildren(item.type) && (
                                <div
                                    className={cn('pl-6 ml-4 border-l transition-all duration-200', {
                            ...
                            <div className='my-2'>
                                <DropIndicator
                                    isActive={isItemTarget && dropPosition === 'after' && activeId !== item.uniqueId}
                                />
                            </div>
                        </React.Fragment>
                    );
    ...
    const rootContainerClasses = cn('p-1 backdrop-blur-sm rounded-2xl transition-all duration-200', {
                    ...
                    <SortableContext
                        items={[
                            ...
                            ...flatItems
                                .filter(
                                    item =>
                                        canHaveChildren(item.type) && (!item.children || item.children.length === 0),
                                )
