...
const selectClassNames: ClassNamesConfig = {
    ...
    control: ({ isFocused, isDisabled, menuIsOpen }) =>
        cn(
            ...
            isFocused
                ? 'border-pumpkin-orange dark:border-pumpkin-orange shadow-sm shadow-pumpkin-orange/20 dark:shadow-pumpkin-orange/10'
                : '',
    ...
    menuList: () => cn('py-1 bg-transparent scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600'),
    ...
    option: ({ isDisabled, isFocused, isSelected }) =>
        cn(
            ...
            isSelected
                ...
                : '',
            !isSelected && isFocused ? 'bg-gray-100/60 dark:bg-gray-700/60' : '',
            isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:translate-y-[-1px]',
        ),

    // Placeholder text
    placeholder: () => cn('text-gray-500 dark:text-gray-400'),

    // Selected value (single select)
    singleValue: () => cn('text-gray-900 dark:text-white'),
    ...
    // Multi-value label
    multiValueLabel: () => cn('px-2 py-0.5 text-sm font-medium text-gray-800 dark:text-gray-100'),

    // Multi-value remove button
    multiValueRemove: () => cn('px-1.5 hover:bg-pumpkin-orange hover:text-white transition-colors', 'rounded-r-lg'),

    // Value container
    valueContainer: () => cn('px-4 py-2 gap-1'),

    // Indicators container
    indicatorsContainer: () => cn('text-gray-500 dark:text-gray-400'),
    ...
    // Indicator separator
    indicatorSeparator: () => cn('bg-gray-300 dark:bg-gray-600 mx-1'),

    // Loading message
    loadingMessage: () => cn('py-3 px-4 text-center text-gray-500 dark:text-gray-400'),

    // No options message
    noOptionsMessage: () => cn('py-3 px-4 text-center text-gray-500 dark:text-gray-400'),

    // Input field
    input: () => cn('text-gray-900 dark:text-white m-0 p-0'),

    // Option group
    group: () => cn('border-gray-200 dark:border-gray-700 border-b py-1'),

    // Group heading
    groupHeading: () =>
        cn('px-4 py-1.5 text-xs font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400'),

    // Menu portal (for rendering outside container)
    menuPortal: () => cn('z-[2000]'),

    // Loading indicator
    loadingIndicator: () => cn('text-pumpkin-orange dark:text-pumpkin-orange'),
};
