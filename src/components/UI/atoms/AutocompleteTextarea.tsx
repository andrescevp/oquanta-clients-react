    ...
    (props, ref) => {
        ...
        const handleSelectPlaceholder = (placeholder: string) => {
            ...
            if (lastInitializerPos !== -1) {
                ...
                const newCursorPos =
                    lastInitializerPos +
                    currentInitializer.start.length +
                    placeholder.length +
                    currentInitializer.stop.length;
            ...
            if (showAutocomplete && filteredPlaceholders.length > 0) {
                switch (e.key) {
                    ...
                    case 'ArrowUp':
                        e.preventDefault();
                        setSelectedIndex(
                            prev => (prev - 1 + filteredPlaceholders.length) % filteredPlaceholders.length,
                        );
                        return;
                    case 'Enter':
                        if (showAutocomplete) {
        ...
        const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
            ...
            if (showAutocomplete && currentInitializer) {
                ...
                if (lastInitializerPos !== -1) {
                    ...
                    if (
                        filtered.length === 0 ||
                        value.substring(cursorPos - currentInitializer.stop.length, cursorPos) ===
                            currentInitializer.stop
                    ) {
                        setShowAutocomplete(false);
                    }
                        ...
                        <ul className='py-1'>
                            {filteredPlaceholders.map((placeholder, index) => (
                                <li
                                    ...
                                    style={{
                                        color: selectedIndex === index ? getPlaceholderColor(placeholder) : undefined,
                                    }}>
                                    {placeholder}
                                </li>
                            ))}
