    ...
    useEffect(() => {
        ...
        const initializeDataPoints = (width: number, height: number) => {
            ...
            if (insightFocus === 'campaigns') {
                typeDistribution.campaign = 0.4;
                typeDistribution.user = 0.25;
            } else if (insightFocus === 'engagement') {
                typeDistribution.user = 0.5;
                typeDistribution.campaign = 0.1;
            } else if (insightFocus === 'conversions') {
                typeDistribution.business = 0.4;
                typeDistribution.tracker = 0.2;
            }
        ...
        const animate = () => {
            ...
            if (dynamic && pointsRef.current.length < maxPointsRef.current) {
                ...
                if (now - lastPointAddedRef.current > 10) {
                    // Add a point every ~10 frames
                    ...
                    pointsRef.current.push(
                        createDataPoint(Math.random() * canvas.width, Math.random() * canvas.height, newType),
                    );
                }
            }
