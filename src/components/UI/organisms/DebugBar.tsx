...
import { ArrowRightIcon, IconChevronDown, IconChevronUp, IconClock, IconExternalLink, IconGripHorizontal, IconTrash as IconTrash2, IconX } from '../Icons';
...
const DebugBar: React.FC = () => {
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              ...
              <div className="bg-gray-900 rounded p-2">
                <div><span className="text-gray-400">Time:</span> {formatTime(request.timestamp)}</div>
                <div><span className="text-gray-400">Token:</span> {request.token}</div>
                <div><span className="text-gray-400">Method:</span> {request.method}</div>
                <div><span className="text-gray-400">URL:</span> {request.url}</div>
            ...
            <div>
              ...
              <div className="bg-gray-900 rounded p-2">
                <div>
                  ...
                  <span className={cn("px-1 rounded ml-1", getStatusColor(request.status), "text-white")}>
                    {request.status}
                  </span>
                </div>
                <div><span className="text-gray-400">Duration:</span> {formatDuration(request.duration)}</div>
                <div><span className="text-gray-400">Size:</span> {formatSize(request.responseSize)}</div>
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-gray-900 text-white border-t border-gray-700 font-mono text-xs shadow-xl z-2000" style={{ height: `${height}px` }}>
      ...
      <div className="overflow-y-auto bg-gray-900" style={{ maxHeight: `${height - 40}px` }}>
        {requests.length === 0 ? (
          <div className="p-4 text-center text-gray-400">
            No hay requests registrados todavía
          </div>
        ) : (
          <div className="divide-y divide-gray-800">
            {requests.map(requestDrawer)}
          </div>
        )}
      </div>
    </div>
