...
function offsetPanelReducer(
  state: OffsetPanelContextState,
  action: OffsetPanelAction
): OffsetPanelContextState {
  switch (action.type) {
    case 'REGISTER_PANEL': {
    ...
    case 'OPEN_PANEL': {
      ...
      return {
        ...
        panels: state.panels.map(panel => 
          panel.id === id
            ? { ...panel, isOpen: true, zIndex: highestZIndex + 1 }
            : panel
        )
      };
    }
...
export const OffsetPanelProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <OffsetPanelContext.Provider value={value}>
      {children}
    </OffsetPanelContext.Provider>
  );
};