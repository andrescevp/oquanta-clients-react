...
export const ORGANIZATION_ROLES: SelectOption[] = [{ value: 'ROLE_OWNER', label: 'Owner' }];
...
const OrganizationUserForm: React.FC<OrganizationUserFormProps> = ({
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || t('No se pudo guardar la información del usuario de organización'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            setError(err.response?.data?.message || t('No se pudo eliminar la relación usuario-organización'));
        } finally {
            setLoading(false);
        }
    };
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select
                                ...
                                className={cn(errors.user && 'border-red-500 ring-1 ring-red-500 rounded-md')}
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select
                                ...
                                className={cn(errors.organization && 'border-red-500 ring-1 ring-red-500 rounded-md')}
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select
                                ...
                                className={cn(errors.roles && 'border-red-500 ring-1 ring-red-500 rounded-md')}
                            />
                        )}
                    />
                    {errors.roles && (
                        ...
                        {id && (
                            <ConfirmationTooltip
                                confirmationMessage={t(
                                    '¿Estás seguro de que deseas eliminar esta relación usuario-organización? Esta acción no se puede deshacer.',
                                )}
                                ...
                                <ButtonLoader
                                    type='button'
                                    className={cn('btn bg-red-600 text-white hover:bg-red-700')}
                                    disabled={loading}
                                    loading={loading}
