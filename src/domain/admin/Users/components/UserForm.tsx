    ...
    <div className="p-6 max-w-2xl">      
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <div className="space-y-2">
            {AVAILABLE_ROLES.map((role) => (
              <div key={role.id} className="flex items-center">
                ...
                />
                <label htmlFor={`role-${role.id}`} className="ml-2 text-sm text-gray-700 dark:text-gray-200">
                  {t(role.label)}
                </label>
              </div>
            ))}
          ...
          {errors.roles && (
            <p className="mt-1.5 text-sm text-red-600 dark:text-red-400">{t(errors.roles.message as string)}</p>
          )}
        </div>
