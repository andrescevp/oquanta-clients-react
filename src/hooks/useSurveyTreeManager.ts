...
const isDescendantOf = (possibleDescendant: string, possibleAncestor: string, items: QuestionItem[]): boolean => {
...
const findLastIndex = (arr: QuestionItem[], predicate: (item: QuestionItem) => boolean): number => {
...
interface UseSurveyTreeManagerReturn {
    ...
    isDescendantOf: (possibleDescendant: string, possibleAncestor: string, flatItems: QuestionItem[]) => boolean;
    handleSelectItem: (item: QuestionItem) => void;
}

export const useSurveyTreeManager = ({
    items,
    onItemsChange,
    onItemSelect,
}: UseSurveyTreeManagerOptions): UseSurveyTreeManagerReturn => {
    ...
    const addSubQuestion = (parentId: string) => {
        ...
        const newSubQuestion: QuestionItem = {
            ...
            parentCodes: parent.parentCodes ? [...parent.parentCodes, parent.code] : [parent.code],
            parentIndexes: parent.parentIndexes ? [...parent.parentIndexes, parent.index] : [parent.index],
        };
    ...
    const handleDeleteItem = (itemCode: string) => {
        const filterItems = (treeItems: QuestionItem[], codeToRemove: string): QuestionItem[] => {
        ...
        const updateParent = (
            ...
            return treeItems.map(item => {
                ...
                if (item.children && item.children.length > 0) {
                    return {
                        ...item,
                        children: updateParent(item.children as QuestionItem[], parentIdToUpdate, newChildToUpdate),
                    };
                }
                return item;
            });
    ...
    const handleDropOnDropzone = (activeSurveyQuestionId: string, parentId: string) => {
        const activeItemIndex = flatItems.findIndex(item => item.uniqueId === activeSurveyQuestionId);
        ...
        if (activeItemIndex >= 0) {
            ...
            childrenToMove.forEach(child => {
                const childInNewList = newFlatItems.find(item => item.uniqueId === child.uniqueId);
                if (childInNewList) {
                    const originalDepthDiff = child.depth - flatItems[activeItemIndex].depth;
    ...
    const handleDropOnItem = (activeId: string, overId: string, dropPosition: 'before' | 'after' | 'inside') => {
        const activeItemIndex = flatItems.findIndex(item => item.uniqueId === activeId);
        ...
        if (activeItemIndex !== -1 && overItemIndex !== -1) {
            ...
            if (dropPosition === 'inside' && isDescendantOf(overItem.uniqueId, activeItem.uniqueId, flatItems)) {
            ...
            switch (dropPosition) {
                case 'before': {
                    newIndex = overItemIndex > activeItemIndex ? overItemIndex - 1 : overItemIndex;
                ...
                case 'after': {
                    newIndex = overItemIndex > activeItemIndex ? overItemIndex : overItemIndex + 1;
                    newParentUniqueId = overItem.parentUniqueId;
                    newDepth = overItem.depth;
                    parentCode = overItem.parentCode;
                    parentIndex = overItem.parentIndex;
                ...
                case 'inside': {
                    ...
                    newIndex = lastChildIndex !== -1 ? lastChildIndex + 1 : newFlatItems.length;
                    parentCode = overItem.code;
                    parentIndex = overItem.index;
                    parentCodes = overItem.parentCodes ? [...overItem.parentCodes, overItem.code] : [overItem.code];
                    parentIndexes = overItem.parentIndexes
