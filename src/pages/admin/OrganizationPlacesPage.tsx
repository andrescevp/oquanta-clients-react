...
import {
    GetApiOrganizationPlacesListOrderEnum,
    Organization,
    OrganizationPlace,
    OrganizationPlaceBasic,
    OrganizationPlaceList,
    OrganizationPlacesApi,
    OrganizationsApi,
...
const OrganizationNameCell = React.memo(
    ({
        organizationUuid,
        organizationsCache,
        getOrganizationName,
...
const OrganizationPlacesPage: React.FC = () => {
    ...
    const [sortOrder, setSortOrder] = useState<GetApiOrganizationPlacesListOrderEnum>(
        GetApiOrganizationPlacesListOrderEnum.Asc,
    );
    ...
    const handleSort = (column: TableColumn<OrganizationPlaceBasic>, sortDirection: string) => {
        ...
        setSortOrder(
            sortDirection.toUpperCase() === 'DESC'
                ? GetApiOrganizationPlacesListOrderEnum.Desc
                : GetApiOrganizationPlacesListOrderEnum.Asc,
        );
    };
                          ...
                          <div>
                              ...
                              {row.uuid && (
                                  <OffsetPanel
                                      ...
                                      panelId={`place_${row.uuid}`}>
                                      <div className='p-2'>
                                          <OrganizationPlaceWorkingHoursManager placeUuid={row.uuid} />
                                      </div>
                                  </OffsetPanel>
                              )}
                              {row.uuid && (
                                  <OffsetPanel
                                      ...
                                      panelId={`place_${row.uuid}`}>
                                      <div className='p-2'>
                                          <OrganizationPlaceSurveys organizationPlaceId={row.uuid} />
                                      </div>
                                  </OffsetPanel>
                              )}
                          </div>
    ...
    return (
        <div className='container mx-auto px-4 py-8'>
            ...
            <div className='mb-4 flex flex-wrap justify-end items-center gap-2'>
                ...
                <button className='btn' onClick={loadPlaces} title={t('Actualizar')}>
                    <IconRefresh size={20} />
                </button>
            </div>
            ...
            <div className='rounded-t shadow-md'>
                <DataTable<OrganizationPlaceBasic>
                    ...
                    noDataComponent={
                        <div className='p-4 text-center text-gray-500'>{t('No hay establecimientos disponibles')}</div>
                    }
                />
            </div>
        </div>
